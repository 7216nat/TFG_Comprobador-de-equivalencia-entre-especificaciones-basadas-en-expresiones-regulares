...
public class Algoritmos {
	...
	public static String detHopKarp(Automata at1, Automata at2, IdEstado idst, ArrayList<String> simb) {
		HashMap<Integer, EstadoDetHK> aExplorar = new HashMap<Integer, EstadoDetHK>();
		...
		EstadoDetHK iniAFD1 = at1.lambdaCierre(inicial, idst.getId());
		if (inicial.esFin())
			iniAFD1.cambioFin(true);
		idst.nextId();
		EstadoDetHK iniAFD2 = at2.lambdaCierre(inicial2, idst.getId());
		if (inicial2.esFin())
		...
		while (!aComparar.isEmpty()) {
			...
			while (it.hasNext()) {
				...
				if (tr1 && tr2) {
					EstadoDetHK estado1 = (EstadoDetHK) afd1.getEstado(dest1);
					EstadoDetHK estado2 = (EstadoDetHK) afd2.getEstado(dest2);
	...
	private static void determinarEstado(int viejo, Automata afd, String simb, IdEstado idst, Automata at,
			HashMap<Integer, EstadoDetHK> aExplorar) {

		EstadoDetHK nuevoEstado = new EstadoDetHK(idst.getId());
		idst.nextId();

		// Para el cada uno de los estados a comparar, saco su trozo del AFD
		EstadoDetHK estado = (EstadoDetHK) afd.getEstado(viejo);
		...
		while (eqIt.hasNext()) {
			...
			while (transIt.hasNext()) {
				...
				if (tAux.getSymb().equals(simb)) {
					EstadoDetHK aux = at.lambdaCierre(at.getEstado(tAux.getEstadoDest()), -1);
		...
		while (noEsta && itAut.hasNext()) {
			...
			EstadoDetHK comp = (EstadoDetHK) par.getValue();
...
//	}
	 ...
	 ***************************/
	public static String derivadasHK(ExpressionBase ex1, ExpressionBase ex2, IdEstado idst,
			ArrayList<String> simb) {
	...
	public static ExpressionBase derivada(ExpressionBase ex, String sim) {
		...
		if (tipo == Tipo.VACIO) {
			...
			return new Lambdaa();
		else if ((tipo == Tipo.SIMB || tipo == Tipo.UNIONRANGOS || tipo == Tipo.RANGO) && ex.get_sim().equals(sim))
			return new Lambdaa();
		else if ((tipo == Tipo.SIMB || tipo == Tipo.UNIONRANGOS || tipo == Tipo.RANGO) && !ex.get_sim().equals(sim))
			return new Vacio();
		// Casos recursivos
